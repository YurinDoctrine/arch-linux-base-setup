...
# Before hop in
sudo pacman -Syy &&
    sudo pacman -S --needed --noconfirm base-devel binutils git gnupg haveged kmod libinput lz4 pacman-contrib pkgconf psmisc ufw wget xdg-utils
...
sudo sed -i -e 's/-j.*/-j$(expr $(nproc) - 1) -l$(expr $(nproc) - 1)"/;s/^#MAKEFLAGS/MAKEFLAGS/;s/^#RUSTFLAGS/RUSTFLAGS/' /etc/makepkg.conf
...
echo -e "Set OPTIONS"
sudo sed -i -e "s|OPTIONS=(strip.*|OPTIONS=(strip docs !libtool !staticlibs emptydirs zipman purge !debug !lto)|g" /etc/makepkg.conf
...
sudo sed -i -e 's| defaults| rw,lazytime,relatime,discard=async,commit=60|g' /etc/fstab
sudo sed -i -e 's| errors=remount-ro| rw,lazytime,relatime,discard=async,commit=60,errors=remount-ro|g' /etc/fstab
